...
void play_smithy(struct gameState *state, int handPos) {
  int currentPlayer = whoseTurn(state);
  //+3 Cards
  // Introduce bug - player draws 2 cards instead of original 3
  int i;
  for (i = 0; i < 2; i++)
  {
...
void play_village(struct gameState *state, int handPos) {
  ...
  // Introduce bug - player gets an additional 3 actions instead of original 2
...
void play_council_room(struct gameState *state, int handPos) {
  ...
  // Introduce bug - player loses a buy
  state->numBuys--;
      
  // Introduce bug - all other players do not get to draw

  //discard played card from hand
  discardCard(handPos, currentPlayer, state, 0);
}

void play_sea_hag(struct gameState *state) {
  int currentPlayer = whoseTurn(state);
  int i;
